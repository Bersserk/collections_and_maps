...
public class BenchmarkFragmentTestTest extends BenchmarkRuleTest {
    ...
    public void test_editText() {
        ...
        inputField.check(matches(ViewMatchers.withText("1000")));
    }
    ...
    private void checkEachItem(int delayBeforeMillis, int list,
                               String textOfItem, String alphaOfItem) {

        List<ResultItem> initedList = (list == COLLECTIONS)
        ...
        if (textOfItem.equals("any") && alphaOfItem.equals("any")) {
            text = Matchers.anyOf(withSubstring("1000.0"), withSubstring(""));
            alpha = Matchers.anyOf(withAlpha(1.0f), withAlpha(0.0f));
        } else {
        ...
        int i = 0;
        for (ResultItem item : initedList) {
            if (item.isHeader()) {
